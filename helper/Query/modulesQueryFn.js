...
const getModules = asyncHandler(async (userId,ParentId) => {
  return await db.query(
    `select modules.id,modules.name,modules.description,order_no,user_score.score,user_score.quiz_attempt,user_score.total_questions from modules left join user_score on modules.id = user_score.module_fk and user_score.user_fk=? where modules.parent_id =?`,
